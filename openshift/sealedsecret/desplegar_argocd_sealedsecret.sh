...
create_sealed_secrets() {
  echo "==> Creando namespace de Sealed Secrets si no existe..."
  kubectl get ns "$SEALED_SECRETS_NAMESPACE" >/dev/null 2>&1 || kubectl create namespace "$SEALED_SECRETS_NAMESPACE"

  echo "==> Añadiendo repo Helm de Sealed Secrets..."
  helm repo add "$SEALED_SECRETS_REPO_NAME" "$SEALED_SECRETS_REPO_URL" >/dev/null 2>&1 || true
  helm repo update >/dev/null

  echo "==> Desplegando Sealed Secrets con Helm..."
  helm upgrade --install "$SEALED_SECRETS_RELEASE" "$SEALED_SECRETS_CHART_NAME" \
    -n "$SEALED_SECRETS_NAMESPACE" \
    --set-string fullnameOverride=sealed-secrets

  echo "==> Esperando deployment de Sealed Secrets..."
  kubectl rollout status deployment/sealed-secrets -n "$SEALED_SECRETS_NAMESPACE" --timeout=300s

  echo "==> Verificacion Sealed Secrets:"
  kubectl get pod,svc,deploy -n "$SEALED_SECRETS_NAMESPACE"
}
...
create_argocd() {
  ...
  echo "==> Desplegando tambien Sealed Secrets..."
  create_sealed_secrets
  ...
  echo "=========================================="
  echo "SEALED SECRETS DESPLEGADO CORRECTAMENTE"
  echo "NAMESPACE:${SEALED_SECRETS_NAMESPACE}"
  echo "RELEASE:  ${SEALED_SECRETS_RELEASE}"
  echo "SERVICE:  sealed-secrets"
  echo "DEPLOY:   sealed-secrets"
  echo "=========================================="
}
...
destroy_argocd() {
  ...
  echo "==> Eliminando release Helm de Argo CD si existe..."
  ...
  echo "==> Eliminando release Helm de Sealed Secrets si existe..."
  if helm status "$SEALED_SECRETS_RELEASE" -n "$SEALED_SECRETS_NAMESPACE" >/dev/null 2>&1; then
    helm uninstall "$SEALED_SECRETS_RELEASE" -n "$SEALED_SECRETS_NAMESPACE"
  else
    echo "Release $SEALED_SECRETS_RELEASE no existe en $SEALED_SECRETS_NAMESPACE"
  fi

  echo "==> Eliminando namespace de Argo CD si existe..."
  ...
  echo "==> Eliminando namespace de Sealed Secrets si existe..."
  if kubectl get ns "$SEALED_SECRETS_NAMESPACE" >/dev/null 2>&1; then
    kubectl delete ns "$SEALED_SECRETS_NAMESPACE"
  else
    echo "Namespace $SEALED_SECRETS_NAMESPACE no existe"
  fi
